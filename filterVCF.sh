...
# better use a vector
v=()
# and vector for patterns
pAR=()
pAD=()
for i in {0..2}; do
	v[$i]=$(grep "##" -v  Trio$1.vcf | grep "#" | cut -f $((i + 10)))
	# check for substring in c10
...
# we use the file sorted that's to the first command of the pipeline
bcftools view -S sortingFile.txt "${filename}${1}.vcf" | bcftools filter -i 'QUAL>20' > "${filename}${1}.qual.vcf" 

## NOW WE JUST CONSIDER THE REGION OF INTERESENT, THE CODING PART ##
